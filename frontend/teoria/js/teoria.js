// ACADEMIA ADDISON - PLATFORM.JS CON FIREBASE GOOGLE AUTH + CLASES EN VIVO
...
auth.onAuthStateChanged(async user=>{
  if(user){
    ...
    state.user=profile;
    state.isTeacher = TEACHER_EMAILS.includes(email);
    enterApp();
...
function openContent(title,url,courseId,topicId,subId,type){
  ...
  if(targetUrl){
    ...
    try{
      ...
      if(studentName){urlWithIds+='&name='+encodeURIComponent(studentName);}
    }catch(e){}
...
function renderLiveBar(){
  ...
  if(session && session.active){
    liveBarEl.innerHTML = `<div class="live-bar-left"><div class="live-dot"></div><div><div class="live-bar-title">Clase en vivo activa</div><div class="live-bar-sub">${course.name} • ${session.participants||0} alumnos conectados</div></div></div><button class="live-bar-btn" onclick="joinLiveClass('${course.id}')">Entrar a la sala</button>`;
  }else{
    liveBarEl.innerHTML = `<div class="live-bar-left"><div class="live-dot" style="background:rgba(255,255,255,.5);animation:none"></div><div><div class="live-bar-title">Iniciar clase en vivo</div><div class="live-bar-sub">${course.name} • Los alumnos verán la invitación al instante</div></div><button class="live-bar-btn" onclick="startLiveClass('${course.id}')">Iniciar ahora</button>`;
  }
}
...
window.startLiveClass = startLiveClass; window.joinLiveClass = joinLiveClass;
