function renderizarSalones({ salones, institucionId, institucionNombre, onCrear, onVer, onEditar, onEliminar, onVolver }) {
  ...
  grid.innerHTML = salones.map(s => `
    <div style="background:var(--superficie);border:1px solid var(--borde);border-radius:var(--radio-borde);padding:16px;cursor:pointer;" data-accion="ver" data-id="${s.salon_id}">
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px;">
        <h3 style="margin:0;font-size:16px;">${s.nombre_salon}</h3>
  ...
  grid.addEventListener('click', (e) => {
    const card = e.target.closest('[data-accion]');
    if (!card) return;
    const { accion, id } = card.dataset;
...
function renderizarDetalleSalon({ salon, usuarios, cursos, usuariosDisponibles, cursosDisponibles, onAsignarUsuario, onQuitarUsuario, onAsignarCurso, onQuitarCurso, onVolver }) {
  ...
  document.getElementById('btnAddUsuario')?.addEventListener('click', () => {
    ...
    const opciones = usuariosDisponibles.map(u => `${u.membresia_id}: ${u.nombre_completo} (${u.correo_electronico})`).join('\n');
