...
async function crearUsuario(req, res) {
  try {
    ...
    if (existente.rows.length > 0) {
      ...
      if (membresia.rows.length > 0) return res.status(400).json({ error: 'Usuario ya en esta institucion', codigo: 'MEMBRESIA_EXISTENTE' });
    ...
    res.status(500).json({ error: 'Error creando usuario', codigo: 'USUARIO_ERROR' });
  }
}
